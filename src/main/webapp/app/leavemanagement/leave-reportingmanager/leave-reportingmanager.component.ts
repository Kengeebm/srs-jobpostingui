...
@Component({
  selector: 'jhi-leave-reportingmanager',
  templateUrl: './leave-reportingmanager.component.html',
  styleUrls: ['./leave-reportingmanager.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class LeaveReportingManagerComponent implements OnInit {
  date = new Date();
  ...
  findAll() {
    if (this.selectedData === 'WFH Submitted List') {
      this.userDetailsLeaveService
        .findListByReportingManager([this.currentAccount.email], [WFH], [PENDING])
        .toPromise()
        .then(value => {
      ...
      this.userDetailsLeaveService
        .findListByReportingManager([this.currentAccount.email], [WFH], [APPROVED])
        .toPromise()
        .then(value => {
      ...
      this.userDetailsLeaveService
        .findListByReportingManager([this.currentAccount.email], [SL, CL], [PENDING])
        .toPromise()
        .then(value => {
      ...
      this.userDetailsLeaveService
        .findListByReportingManager([this.currentAccount.email], [SL, CL], [APPROVED])
        .toPromise()
        .then(value => {
      ...
      this.userDetailsLeaveService
        .findListByReportingManager([this.currentAccount.email], [WCL, CO], [PENDING])
        .toPromise()
        .then(value => {
      ...
      this.userDetailsLeaveService
        .findListByReportingManager([this.currentAccount.email], [WCL, CO], [APPROVED])
        .toPromise()
        .then(value => {
      ...
      this.userDetailsLeaveService
        .findRejectedListByReportingManager([this.currentAccount.email], ['Reject'])
        .toPromise()
        .then(value => {
        ...
        .then(result => {
          if (result.value) {
            ...
            for (const value of Object.values(this.selection.selected)) {
              value.approvedBy = this.currentAccount.login;
